...
class SimpleChart: UIView, CAAnimationDelegate {
    ...
    weak var delegate: SimpleChartDelegate?
    ...
    func setupChartWith(labels: [String], data: [Double]) {
        ...
        if labels.count != data.count { interactiveLayer.isUserInteractionEnabled = false
            print("Number of labels do not match the number of data points")
            return
        }
        ...
        for labelCount in 0..<labels.count {

            //computed property of xvalue
            var xValue: CGFloat {
                if labelCount != 0 { return (CGFloat(labelCount) * labelWidth)
                } else {
                    return 0
                }
            }
            ...
            dataLabel.text = labels[labelCount]
            ...
            //calculate the center point of the label and adjust the plot value,
            //as each plot will be centered to the label.
            let halfWidth = labelWidth / 2

            //**==Graph Plot Setup==**\\
            //work out value percentage
            let valuePercentage = CGFloat((data[labelCount] - lowestValue) / difference * 100)
    ...
    @objc private func handleTap(sender: UITapGestureRecognizer) {
        ...
        for point in labelPoints where point.x...(point.x + labelWidth) ~= currentPosition.x {

            //get the tapped index
            guard let tappedIndex = labelPoints.firstIndex(of: point) else {
                return
            }
                ...
                if var currentIndex = currentIndex {
                    ...
                    let isPlus: Bool = currentIndex < tappedIndex ? true : false
                    //zeroed difference property - will be updated later depdning on tap direction
                    let difference: Int = isPlus ? tappedIndex - currentIndex : currentIndex - tappedIndex
                    ...
                    var animationPlots: [CGPoint] = [CGPoint(x: graphPlots[currentIndex].x,
                                                             y: graphPlots[currentIndex].y)]
                    //loop through the plots between the current index and the tapped index - add to array
                    for value in 1...difference {
                        if isPlus {
                            currentIndex += 1
                        } else if !isPlus {
                            currentIndex -= 1
                        }
                        animationPlots.append(CGPoint(x: graphPlots[currentIndex].x,
                                                      y: graphPlots[currentIndex].y))
                        if value == difference {
                            newpoint = graphPlots[currentIndex]
                        }
                    }
                ...
                //interaction not initiated yet
                    ...
                    //set the new origin
                    selectedPoint.frame.origin = CGPoint(x: graphPlots[tappedIndex].x - 10,
                                                         y: graphPlots[tappedIndex].y - 10)
                ...
                UIView.animate(withDuration: 0.5, delay: 0.0, usingSpringWithDamping: 0.6,
                               initialSpringVelocity: 0.8, options: UIView.AnimationOptions.curveLinear,
                               animations: { () -> Void in
    ...
    // MARK: - Clear Graph
    ///Clears the graph when it is about to be reused.
    func clearGraph() {
